                    ...
                    with patch("json.dump"):
                        with patch("os.path.exists", return_value=True):
                            with patch("builtins.open", MagicMock()):
                                ...
                                mock_freesurfer_wrapper.process_subject.assert_called_once_with(
                                    "01", None, mock_layout
                                )
                ...
                with patch("pathlib.Path.exists", return_value=True):
                    with patch("builtins.open", MagicMock()):
                        with patch("rdflib.Graph.serialize"):
                            ...
                            # Check that the right functions were called
                            mock_bids_provenance.create_group_provenance.assert_called_once_with(
                                ["sub-01", "sub-02"]
                            )
                            mock_create_group_nidm.assert_called_once_with(
                                ["sub-01", "sub-02"], "/output/nidm"
                            )
...
def test_copy_freesurfer_outputs(mock_copy2, mock_makedirs, mock_exists):
    ...
    copy_freesurfer_outputs("/output/freesurfer", "01", None, "/output/sub-01")
    ...
    copy_freesurfer_outputs("/output/freesurfer", "01", "01", "/output/sub-01/ses-01")
