...
class FreeSurferWrapper:
    ...
    def _setup_freesurfer_env(self):
        ...
        if self.freesurfer_license:
            if os.path.exists(self.freesurfer_license):
                ...
                logger.error(f"FreeSurfer license not found at specified path: {self.freesurfer_license}")
    ...
    def process_subject(self, subject_label, session_label=None, layout=None):
        ...
        logger.info(f"Processing subject sub-{subject_label}{' session ses-' + session_label if session_label else ''}")
        ...
        try:
            ...
            if not t1w_images:
                logger.error(f"No T1w images found for sub-{subject_label}{' ses-' + session_label if session_label else ''}")
            ...
            if t2w_images:
                logger.info(f"Found {len(t2w_images)} T2w images for sub-{subject_label}{' ses-' + session_label if session_label else ''}")
            
            # Set up subject ID and output directory
            if session_label:
                subject_id = f"{subject_label}_{session_label}"
                bids_output_dir = self.output_dir / f"sub-{subject_label}" / f"ses-{session_label}"
            else:
            ...
            if subject_dir.exists() and (subject_dir / 'scripts' / 'recon-all.done').exists():
    ...
    def _find_t1w_images(self, layout, subject_label, session_label=None):
        ...
        logger.info(f"Found {len(files)} T1w images for sub-{subject_label}{' ses-' + session_label if session_label else ''}")
        for f in files:
...
def main(bids_dir, output_dir, analysis_level, participant_label, session_label, 
         freesurfer_license, skip_bids_validator, fs_options, verbose):
    """
