            ...
            try:
                container_id = (
                    subprocess.check_output(
                        ["cat", "/proc/self/cgroup"], universal_newlines=True
                    )
                    .split("\n")[0]
                    .split("/")[-1]
                )

                return {"Type": "docker", "ID": container_id}
            except:
                return {"Type": "docker"}
...
"""
    ...
    def create_subject_provenance(
        ...
        try:
            logger.info(
                f"Creating provenance for subject {subject_label}{' session ' + session_label if session_label else ''}"
            )
            ...
            if t1w_files:
                for file_path in t1w_files:
                    try:
                        ...
                        input_files.append(
                            {"path": str(file_path), "type": "T1w", "hash": file_hash}
                        )
                    except Exception as e:
                        logger.warning(f"Failed to hash file {file_path}: {str(e)}")
                        input_files.append({"path": str(file_path), "type": "T1w"})
            ...
            if t2w_files:
                for file_path in t2w_files:
                    try:
                        file_hash = self._calculate_file_hash(file_path)
                        input_files.append(
                            {"path": str(file_path), "type": "T2w", "hash": file_hash}
                        )
                    except Exception as e:
                        logger.warning(f"Failed to hash file {file_path}: {str(e)}")
                        input_files.append({"path": str(file_path), "type": "T2w"})
            ...
            provenance = {
                ...
                "Parameters": {"UseT2": bool(t2w_files)},
            ...
            logger.error(
                f"Failed to create provenance for subject {subject_id}: {str(e)}"
            )
            return {"Error": str(e)}
...
if __name__ == "__main__":
    ...
    try:
        ...
        provenance = create_bids_provenance(
            args.bids_dir, args.output_dir, args.version
        )
